...
```
### Syntax (문법/자료구조)
- ArrayDeque를 Deque로 사용하여 뱀 몸통을 O(1) 시간에 머리 추가, 꼬리 제거
- map 하나로 빈 칸(0), 사과(1), 뱀 몸통(2) 상태를 통합 관리
- 방향 전환은 List<Dir> + index로 순차적 O(1) 접근
- dx/dy 방향 배열은 → ↓ ← ↑ 순서, dir은 (dir+1)%4, (dir+3)%4로 회전
- 1-based map 사용으로 index-1 변환 생략

### Semantic (알고리즘/문제해결)
- 뱀 머리/꼬리 순서를 Deque로 정확히 관리 ‼️‼️
- map으로 충돌과 상태 갱신을 처리 
- 방향 전환은 시간 비교 + index로 최적 처리
- 사과 있으면 꼬리 유지, 없으면 꼬리 제거
- 시간(sec)은 루프마다 1씩 증가, 충돌 시 출력
- 방향 배열과 회전 공식을 반드시 일치시켜야 조기 충돌 방지
```
